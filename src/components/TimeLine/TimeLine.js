...
const Timeline = () => {
  ...
  return (
    <Section id="about">
        ...
        <>
          {TimeLineData.map((item, i) => (
            <CarouselMobileScrollNode key="i" final={i === TOTAL_CAROUSEL_COUNT - 1}>
              <CarouselItem index={i} id={`carouse__item-${i}`} active={activeItem} onClick={(e) => handleClick(e, i)}>
                <CarouselItemTitle>
                  {item.year}
                  <CarouselItemImg
                    width="208"
      ...
      <CarouselButtons>
        {TimeLineData.map((item, i) => (
          <CarouselButton index={i} key={i} active={activeItem} onClick={(e) => handleClick(e, i)} type="button">
